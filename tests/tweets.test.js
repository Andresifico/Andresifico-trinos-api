...
describe('Tweets routes', () => {
  ...
  it('Should return an incorrect request when obtained the information without token', async () => {
    const response = await request(app).get(TWEETS_PATH);

    expect(response.statusCode).toBe(401);
    expect(response.body.status).toBe('Access token required');
  });
  it('Should return wrong request when post tweet without token', async () => {
    const payload = {
      text: 'My tweet',
    };
  ...
  it('Should create tweet', async () => {
    ...
    const response = await request(app).post(TWEETS_PATH)
      .send(payload)
  ...
  it('Should return Tweet not found on post coments tweet', async () => {
    const payload = {
      text: 'My comment #1',
    };
  ...
  it('Should return Access token required on coments tweet', async () => {
    const payload = {
      text: 'My comment #1',
    };
  ...
  it('Should return User not found on user feed', async () => {
    const response = await request(app).get(`${TWEETS_PATH}/feed/amezaf`);
